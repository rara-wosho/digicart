<?php
    ...
    if(!isset($_SESSION['current_user']) || $_SESSION['current_user']['role'] !== "admin"){
        ...
        if(isset($_GET['search-user'])){
            $searchTerm = $_GET['search-user'];
            $users = $user->searchUser($connection, $searchTerm) ;
        }
    }
...
<!DOCTYPE html>
    ...
    <body>
        <div class="admin-dashboard-page expand-sidebar">
            ...
            >
                ...
                <a style="background-color:rgb(240,240,245)" href="logout-logic.php" class="rounded-3 d-flex  align-items-center text-black py-2 px-3"
                    ...
                    <p class="sidebar-label mb-0 text-danger">Log Out</p>
                </a>
            </div>
        ...
        <div class="m-3 bg-white p-4 rounded shadow-sm">
            ...
            <h5 class="mb-0 text-muted text-nowrap text-primary">Users: <?=$users->num_rows?></h5>
            <form class="input-group w-50">
                <input value="<?=$searchTerm?>" name="search-user" type="text" placeholder="Search users..." class="form-control">
                <button type="submit" class="input-group-text bg-primary text-white">Search</button>
                <a href="manage_users.php" class="input-group-text bg-danger text-white">Reset</a>
            </form>
            </div>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td>
                            <?= $row['isBan'] ? 
                                '<a href="banning-logic.php?user_id='.$row['user_id'].'&isBan=0" class="btn btn-danger btn-sm">Banned</a>'
                                : '<a href="banning-logic.php?user_id='.$row['user_id'].'&isBan=1" class="btn btn-outline-danger btn-sm">Ban</a>'
                            ?>
                            </td>
                        </tr>
                    <?php
